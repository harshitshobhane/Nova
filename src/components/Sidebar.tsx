...
import { Home, QrCode, Send, Users, FileText, ShoppingBag, Utensils, Car, Phone, ChevronLeft, Menu, ListChecks, BarChart3, UserCircle } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ theme = 'light' }) => {
  ...
  return (
    <>
      {/* Mobile Hamburger */}
      <button className={`md:hidden fixed top-4 left-4 z-40 p-2 rounded ${bg} ${text} border ${border}`} onClick={() => setMobileOpen(true)} aria-label="Open sidebar">
        <Menu size={28} />
      </button>
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 h-screen transition-all duration-300 flex flex-col
          ...
          <nav className="flex-1 px-2 py-6 space-y-8 overflow-y-auto custom-scrollbar min-h-0">
            <div>
              ...
              <ul className="space-y-1">
                <li><NavLink to="/dashboard" className={navLinkClass(theme)}>{<Home className="h-5 w-5 mr-2" />}{collapsed ? '' : 'Dashboard'}</NavLink></li>
